...
UCLASS()
class MYPROJECT_API Amac : public AActor
{
	GENERATED_BODY()
...
public:
	...
	static void DES_Encrypt(unsigned char* InData, unsigned char* key, unsigned char* OutData)
	//参数:
	...
	static void xor(unsigned char* input1, unsigned char* input2, unsigned char* output, int len)
	{
		while (len)
		{
	...
	*/
	...
	static void MacArithmetic(unsigned char* sMacKey, unsigned char* pInData, int iInLen, unsigned char* pRetData)
	{
		//MAC算法:
	...
	int main()
	{
		...
		unsigned char pInData[19] = {
			0x00, 0x00, 0x12, 0x34, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x20, 0x12, 0x12, 0x07, 0x16, 0x41, 0x00,
			0x80
		}; //加密数据的定义,加密数据定长.
	...
	UFUNCTION(BlueprintCallable, Category = mac)
		static FString encryptMAC(FString mac);
