...
// Called every frame
void Amd5::Tick(float DeltaTime)
{
...
// 幻数定义
const int Amd5::kA = 0x67452301;
const int Amd5::kB = 0xefcdab89;
const int Amd5::kC = 0x98badcfe;
const int Amd5::kD = 0x10325476;
 
const unsigned long long Amd5::k_ti_num_integer = 4294967296;
 
// function: CycleMoveLeft
// @param src_num:要左移的数
// @param bit_num_to_move:要移动的bit位数
// @return  循环左移后的结果数
UInt32 Amd5::CycleMoveLeft(UInt32 src_num, int bit_num_to_move) {
...
UInt32 Amd5::FillData(const char *in_data_ptr, int data_byte_len, char** out_data_ptr) {
...
void Amd5::RoundF(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
...
void Amd5::RoundG(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
    UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
    int s[] = { 5, 9, 14, 20 };
    for (int i = 0; i < 16; ++i) {
...
void Amd5::RoundH(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
    UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
    int s[] = { 4, 11, 16, 23 };
    for (int i = 0; i < 16; ++i) {
...
void Amd5::RoundI(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
    UInt32 *M = reinterpret_cast<UInt32*>(data_BIT_OF_GROUP_ptr);
    int s[] = { 6, 10, 15, 21 };
    for (int i = 0; i < 16; ++i) {
...
void Amd5::RotationCalculate(char *data_512_ptr, ParamDynamic & param) {
...
// 转换成十六进制字符串输出
std::string Amd5::GetHexStr(unsigned int num_str) {
    std::string hexstr = "";
...
// return :加密后的MD5值
std::string Amd5::Encode(std::string src_info) {
    ParamDynamic param;
    param.ua_ = kA;
    param.ub_ = kB;
    param.uc_ = kC;
...
//MD5加密
FString Amd5::encodeMD5(FString string)
{
    std::string cstr = TCHAR_TO_UTF8(*string);
    std::string rcstr = Encode(cstr);

    return UTF8_TO_TCHAR(rcstr.c_str());
}
