...
UMySQLConnection* UMySQLDatabase::Connection(FString Host, FString UserName, FString UserPassword, FString DatabaseName)
{
		...
		0, nullptr, 0))
	{
		...
		UMySQLConnection::CloseConnection(cs);
		return nullptr;
	}
...
bool UMySQLDatabase::ExecuteQuery(FString Query, UMySQLConnection* Connection)
{
	/*	FString qwe = "";
	...
	//if (mysql_query(con, "INSERT INTO `test`.`t1` (`Qwe`) VALUES ('����ڧӧ֧� ');")) {
...
bool UMySQLDatabase::DropTable(const FString TableName, UMySQLConnection* Connection)
{
	...
	idxCrSts = ExecuteQuery(Query, Connection);
	...
	const TArray<FMySQLConnectorTableField> Fields, UMySQLConnection* Connection)
{
	...
	t.Created = ExecuteQuery(query, Connection);

	return t;
}

FMySQLConnectorTableField UMySQLDatabase::INT(const FString FieldName, const bool PK, const bool AI)
{
...
FMySQLConnectorTableField UMySQLDatabase::VARCHAR(const FString FieldName, const FString FieldLength,
...
bool UMySQLDatabase::InsertTest(const FString Query, UMySQLConnection* Connection)
{
...
bool UMySQLDatabase::GetControllerIP(FString& IP, int32& IP_int, APlayerController* PlayerController)
{
	if (PlayerController)
	{
...
MySQLConnectorQueryResult UMySQLDatabase::RunQuery(FString Query, UMySQLConnection* Connection)
{
...
FMySQLConnectoreQueryResult UMySQLDatabase::GetData(const FString& Query, UMySQLConnection* Connection)
{
	...
	MySQLConnectorQueryResult queryResult = RunQuery(Query, Connection);
