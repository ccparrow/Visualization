...
void ATcp::Connect(const FString& ipAddress, int32 port, const FTcpSocketDisconnectDelegate& OnDisconnected, const FTcpSocketConnectDelegate& OnConnected,
	const FTcpSocketReceivedMessageDelegate& OnMessageReceived, int32& ConnectionId)
{
	...
	TWeakObjectPtr<ATcp> thisWeakObjPtr = TWeakObjectPtr<ATcp>(this);
...
void ATcp::Disconnect(int32 ConnectionId)
{	
...
bool ATcp::SendData(int32 ConnectionId /*= 0*/, TArray<uint8> DataToSend)
{
...
void ATcp::ExecuteOnMessageReceived(int32 ConnectionId, TWeakObjectPtr<ATcp> thisObj)
{
...
TArray<uint8> ATcp::Concat_BytesBytes(TArray<uint8> A, TArray<uint8> B)
{
...
TArray<uint8> ATcp::Conv_IntToBytes(int32 InInt)
{
	TArray<uint8> result;
	for (int i = 0; i < 4; i++)
	{
...
TArray<uint8> ATcp::Conv_StringToBytes(const FString& InStr)
{
...
TArray<uint8> ATcp::Conv_FloatToBytes(float InFloat)
{
	TArray<uint8> result;
...
TArray<uint8> ATcp::Conv_ByteToBytes(uint8 InByte)
{
	TArray<uint8> result{ InByte };
	return result;
}

int32 ATcp::Message_ReadInt(TArray<uint8>& Message)
{
	if (Message.Num() < 4)
	{
...
uint8 ATcp::Message_ReadByte(TArray<uint8>& Message)
{
	if (Message.Num() < 1)
	{
...
bool ATcp::Message_ReadBytes(int32 NumBytes, TArray<uint8>& Message, TArray<uint8>& returnArray)
{
...
float ATcp::Message_ReadFloat(TArray<uint8>& Message)
{
	if (Message.Num() < 4)
	{
...
FString ATcp::Message_ReadString(TArray<uint8>& Message, int32 BytesLength)
{
	if (BytesLength <= 0)
	{
		if (BytesLength < 0)
...
bool ATcp::isConnected(int32 ConnectionId)
{
	if (TcpWorkers.Contains(ConnectionId))
...
void ATcp::PrintToConsole(FString Str, bool Error)
{
...
void ATcp::ASCII2HEX(int ASCII, FString& HEX)
{
	...
	while (ASCII&&count++<8)
	{
		s_value = hex_string[(ASCII & 0x0f)]+s_value;
		ASCII >>= 4;
	}
	HEX = s_value.c_str();
}

void ATcp::HEX2ASCII(FString HEX, TArray<uint8>& ASCII)
{
...
void ATcp::ExecuteOnConnected(int32 WorkerId, TWeakObjectPtr<ATcp> thisObj)
{
	if (!thisObj.IsValid())
		return;
...
void ATcp::ExecuteOnDisconnected(int32 WorkerId, TWeakObjectPtr<ATcp> thisObj)
{
	if (!thisObj.IsValid())
		return;
...
FTcpSocketWorker::FTcpSocketWorker(FString inIp, const int32 inPort, TWeakObjectPtr<ATcp> InOwner, int32 inId, int32 inRecvBufferSize, int32 inSendBufferSize, float inTimeBetweenTicks)
	: ipAddress(inIp)
	, port(inPort)
...
{
	AsyncTask(ENamedThreads::GameThread, []() {	ATcp::PrintToConsole("Tcp socket thread was destroyed.", false); });
	Stop();
	if (Thread)
	{
...
{
	AsyncTask(ENamedThreads::GameThread, []() {	ATcp::PrintToConsole("Starting Tcp socket thread.", false); });
	...
	while (bRun)
	{
		...
		if (!bConnected)
		{
			...
			{
				AsyncTask(ENamedThreads::GameThread, []() { ATcp::PrintToConsole(FString::Printf(TEXT("Couldn't connect to server. TcpSocketConnection.cpp: line %d"), __LINE__), true); });
				bRun = false;				
			}
			continue;
		}
		...
		if (!Socket)
		{
			AsyncTask(ENamedThreads::GameThread, []() { ATcp::PrintToConsole(FString::Printf(TEXT("Socket is null. TcpSocketConnection.cpp: line %d"), __LINE__), true); });
			bRun = false;
			continue;
		}
		...
		while (bRun)
		{
			...
			AsyncTask(ENamedThreads::GameThread, []() { ATcp::PrintToConsole("Pending data", false); });
			...
			if (!Socket->Recv(receivedData.GetData() + BytesReadTotal, PendingDataSize, BytesRead))
			{
				...
				AsyncTask(ENamedThreads::GameThread, []() {
					ATcp::PrintToConsole(FString::Printf(TEXT("In progress read failed. TcpSocketConnection.cpp: line %d"), __LINE__), true);
				});
				break;
			}
